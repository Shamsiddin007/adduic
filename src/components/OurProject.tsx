...
//                 {" "}
                // <div className="group relative h-[400px] w-full rounded-2xl bg-gradient-to-b from-black to-gray-800 p-5 text-white shadow-lg transition-all duration-300 hover:shadow-xl">
                //   {" "}
...
function ProjectsPage() {
  ...
  useEffect(() => {
    fetch("https://leuscgqzalmrfujkzpbd.supabase.co/storage/v1/object/sign/ourproject/singleImg/projects.json?token=eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJ1cmwiOiJvdXJwcm9qZWN0L3NpbmdsZUltZy9wcm9qZWN0cy5qc29uIiwiaWF0IjoxNzMxMjYzNDQ0LCJleHAiOjE3NjI3OTk0NDR9.LR1i1jdY-dZ1LUdenb9T8oKsJa_dtJNQ6pcmg2njGZM&t=2024-11-10T18%3A30%3A45.640Z")
      .then((response) => response.json())
      .then((data) => {
        const limitedProjects = data.ourprojects.slice(0, 4);
        setProjects(limitedProjects);
    ...
    <section className="w-full py-12 sm:py-16 lg:py-20">
      <div className="container m-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 mb-10">
          ...
          <Link href="/portfolio">
            <button className="flex items-center justify-center gap-2 rounded-full bg-[#FFFFFF1A] px-6 py-3 text-white hover:bg-[#3375F6] transition-all duration-300">
              All projects <Image src={Chevron} alt="Chevron right" className="w-4 h-4" />
            </button>
          </Link>
        </div>
        ...
        {isMobile ? (
          ...
          >
            {projects.map((project) => (
              <SwiperSlide key={project.id}>
                <Link href={`/portfolio/${project.id}`}>
                  <div className="group relative w-full rounded-2xl bg-gradient-to-b from-black to-gray-800 p-5 text-white shadow-lg transition-all duration-300 hover:shadow-xl">
                    <div className="flex h-full flex-col items-center text-center">
                      <h3 className="text-xl font-bold mb-2">{project.title}</h3>
                      <p className="text-gray-300 pb-14 duration-1000 group-hover:pb-[40px]">{project.date}</p>
                      <div className="flex-grow relative w-full">
                        <img
                          src={project.imgLogo}
                          alt={project.title}
          ...
          <div className="grid grid-cols-4 justify-between gap-8">
            {projects.map((project) => (
                <Link href={`/portfolio/${project.id}`}>
                <div className="group relative h-[400px] w-full overflow-hidden rounded-2xl bg-gradient-to-b from-black to-gray-800 p-5 text-white shadow-lg transition-all duration-300 hover:shadow-xl">
                <div className="relative z-10 flex h-full flex-col items-center text-center">
                  <h3 className="text-[16px] font-bold mb-2">{project.title}</h3>
                  <p className="text-gray-300 pb-14 duration-1000 group-hover:pb-[40px]">
                    {project.date}
                  </p>
